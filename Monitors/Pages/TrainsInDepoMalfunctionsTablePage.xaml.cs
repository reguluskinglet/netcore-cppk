...
namespace WpfMultiScreens.Pages
{
    ...
    /// </summary>
    public partial class TrainsInDepoMalfunctionsTablePage : IReportPage
    {
        private readonly string _depoName;
        private readonly int _depoStantionId;
        public ScreenType ScreenType { get; set; } = ScreenType.TrainsInDepoMalfunctionsTable;

        private readonly int _fullRefreshInterval;
        private const int PageSize = 9;
        private int _page = 1;
        private readonly Timer _pagingTimer;
        private TrainsInDepoMalfunctionTableDto _lastResult;
        private readonly object _pagingTimerLocker = new object();

        private readonly Logger _logger = LogManager.GetCurrentClassLogger();

        public TrainsInDepoMalfunctionsTablePage(int refreshInterval)
        {
            InitializeComponent();

            _depoName = DataRepository.GetInstance().DepoName;
            _depoStantionId = DataRepository.GetInstance().DepoStantionId;
        ...
        public void UpdateData()
        {
            ...
            lock (_pagingTimerLocker)
            {
                try
                {
                    var res = DataRepository.GetInstance().GetTrainsInDepoDepoMalfunctionsTable(_depoStantionId);
                    ...
                    Dispatcher.Invoke(() =>
                    {
                        TableName.Text = "Количество неисправностей на составах, запланированных для ремонтных работ и ТО";
