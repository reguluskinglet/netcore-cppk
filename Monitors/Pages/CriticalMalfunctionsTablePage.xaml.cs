...
namespace WpfMultiScreens.Pages
{
    ...
    /// </summary>
    public partial class CriticalMalfunctionsTablePage : IReportPage
    {
        private readonly string _depoName;
        public ScreenType ScreenType { get; set; } = ScreenType.CriticalMalfunctionsTable;

        private readonly int _fullRefreshInterval;
        private const int PageSize = 9;
        private int _page = 1;
        private readonly Timer _pagingTimer;
        private CriticalMalfunctionsTableDto _lastResult;
        private readonly object _pagingTimerLocker = new object();

        private readonly Logger _logger = LogManager.GetCurrentClassLogger();

        public CriticalMalfunctionsTablePage(int refreshInterval)
        {
            InitializeComponent();
        ...
        public void UpdateData()
        {
            ...
            lock (_pagingTimerLocker)
            {
                try
                {
                    var res = DataRepository.GetInstance().GetCriticalMalfunctionsTable();
                    ...
                    Dispatcher.Invoke(() =>
                    {
                        TableName.Text = "Перечень составов с неисправностями";
