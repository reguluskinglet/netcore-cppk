...
namespace WpfMultiScreens.Pages
{
    ...
    /// </summary>
    public partial class TrainsInDepoStatusTablePage : IReportPage
    {
        private readonly string _depoName;
        private readonly int _depoStantionId;
        public ScreenType ScreenType { get; set; } = ScreenType.TrainsInDepoStatusTable;

        private readonly int _fullRefreshInterval;
        private const int PageSize = 9;
        private int _page = 1;
        private readonly Timer _pagingTimer;
        private TrainsInDepoStatusTableDto _lastResult;
        private readonly object _pagingTimerLocker = new object();

        private readonly Logger _logger = LogManager.GetCurrentClassLogger();

        public TrainsInDepoStatusTablePage(int refreshInterval)
        {
            InitializeComponent();
        ...
        public void UpdateData()
        {
            ...
            lock (_pagingTimerLocker)
            {
                try
                {
                    var res = DataRepository.GetInstance().GetTrainsInDepoStatusTable(_depoStantionId);
                    ...
                    Dispatcher.Invoke(() =>
                    {
                        TableName.Text = "Состояние составов в депо";
