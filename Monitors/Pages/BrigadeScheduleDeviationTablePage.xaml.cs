...
namespace WpfMultiScreens.Pages
{
    ...
    /// </summary>
    public partial class BrigadeScheduleDeviationTablePage : IReportPage
    {
        private readonly string _depoName;
        public ScreenType ScreenType { get; set; } = ScreenType.BrigadeScheduleDeviationTable;

        private readonly int _fullRefreshInterval;
        private const int PageSize = 9;
        private int _page = 1;
        private readonly Timer _pagingTimer;
        private BrigadeScheduleDeviationTableDto _lastResult;
        private readonly object _pagingTimerLocker = new object();

        private readonly Logger _logger = LogManager.GetCurrentClassLogger();

        public BrigadeScheduleDeviationTablePage(int refreshInterval)
        {
            InitializeComponent();
        ...
        public void UpdateData()
        {
            ...
            lock (_pagingTimerLocker)
            {
                try
                {
                    var res = DataRepository.GetInstance().GetBrigadeScheduleDeviationTable();
                    ...
                    Dispatcher.Invoke(() =>
                    {
                        TableName.Text = "Расхождение фактического и планового графика для сотрудников бригад";
