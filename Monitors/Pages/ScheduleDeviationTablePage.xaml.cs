...
namespace WpfMultiScreens.Pages
{
    ...
    /// </summary>
    public partial class ScheduleDeviationTablePage : IReportPage
    {
        private readonly string _depoName;
        public ScreenType ScreenType { get; set; } = ScreenType.ScheduleDeviationTable;

        private readonly int _fullRefreshInterval;
        private const int PageSize = 9;
        private int _page = 1;
        private readonly Timer _pagingTimer;
        private ScheduleDeviationTableDto _lastResult;
        private readonly object _pagingTimerLocker = new object();

        private readonly Logger _logger = LogManager.GetCurrentClassLogger();

        public ScheduleDeviationTablePage(int refreshInterval)
        {
            InitializeComponent();
        ...
        public void UpdateData()
        {
            ...
            lock (_pagingTimerLocker)
            {
                try
                {
                    var res = DataRepository.GetInstance().GetScheduleDeviationTable();
                    ...
                    Dispatcher.Invoke(() =>
                    {
                        TableName.Text = "Расхождение фактического и планового графика рейсов на маршрутах";
