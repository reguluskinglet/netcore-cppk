...
{
    /// <summary>
    /// Логика взаимодействия для JournalsTablePage.xaml
    /// </summary>
    public partial class JournalsTablePage : IReportPage
    {
        private readonly string _depoName;
        private readonly int _depoStantionId;
        public ScreenType ScreenType { get; set; } = ScreenType.JournalsTable;

        private readonly int _fullRefreshInterval;
        private const int PageSize = 9;
        private int _page = 1;
        private readonly Timer _pagingTimer;
        private JournalsTableDto _lastResult;
        private readonly object _pagingTimerLocker = new object();

        private readonly Logger _logger = LogManager.GetCurrentClassLogger();

        public JournalsTablePage(int refreshInterval)
        {
            InitializeComponent();
        ...
        private void OnPagingTimer(object sender, ElapsedEventArgs e)
        {
            if (_lastResult != null)
            {
                ...
                {
                    var pages = _lastResult.Items.Count() / PageSize + 1;
        ...
        public void UpdateData()
        {
            ...
            lock (_pagingTimerLocker)
            {
                try
                {
                    var res = DataRepository.GetInstance().GetJournalsTable(_depoStantionId);
                    ...
                    //var pages = res.Items.Count() / PageSize + 1;
